...
class DSISurrogate:
    ...
    def _solve_map_analytical(self, h_target_scaled, noise_variance):
        lhs = self.surrogate_components_['M_obs'].T @ self.surrogate_components_['M_obs'] + \
              noise_variance * np.identity(self.surrogate_components_['n_components'])
        rhs = self.surrogate_components_['M_obs'].T @ \
              (h_target_scaled - self.surrogate_components_['mean_o'])
        try:
            # Using solve with assume_a='pos' (positive definite)
            x_map = solve(lhs, rhs, assume_a='pos')
        except np.linalg.LinAlgError:
             print("Warning: Analytical solve failed. Returning prior mean.")
             x_map = np.zeros(self.surrogate_components_['n_components'])
        return x_map

    def _solve_map_cg(self, h_target_scaled, noise_variance):
        ...
        def objective_function(x):
            ...
            return 0.5 * (inv_noise_var * np.sum((h_target_scaled - o_pred_scaled)**2) + np.sum(x**2))
        def objective_gradient(x):
            o_pred_scaled = mu_o_cg + M_obs_cg @ x
            return -inv_noise_var * M_obs_cg.T @ (h_target_scaled - o_pred_scaled) + x
        x0 = np.zeros(self.surrogate_components_['n_components'])
    ...
    def _solve_map_gd(self, h_target_scaled, noise_variance, learning_rate=1e-7, n_iterations=5000, tolerance=1e-7):
        inv_noise_var = 1.0 / noise_variance
        x_k = np.zeros(self.surrogate_components_['n_components'])
        M_obs_gd = self.surrogate_components_['M_obs']
        mu_o_gd = self.surrogate_components_['mean_o']
        converged = False
        for k in range(n_iterations):
            if not np.isfinite(x_k).all(): break
            o_pred_scaled = mu_o_gd + M_obs_gd @ x_k
            gradient = -2.0 * inv_noise_var * M_obs_gd.T @ (h_target_scaled - o_pred_scaled) + 2.0 * x_k
            x_k_next = x_k - learning_rate * gradient
            if np.linalg.norm(x_k_next - x_k) < tolerance:
                converged = True; x_k = x_k_next; break
            x_k = x_k_next
        if not converged: print("Warning: GD did not converge.")
        return x_k

    def _solve_map_ls(self, h_target_scaled, noise_variance):
    ...
    # --- Ensemble Solvers with Stability Optimization ---
    ...
    def predict(self, h_observed, obs_noise_std,
        ...
        solver_map = {
            ...
            'gd': lambda h, nv: self._solve_map_gd(h, nv, learning_rate=gd_learning_rate),
            'ls': self._solve_map_ls
        }
        if solver not in solver_map:
            raise ValueError(f"Invalid solver type '{solver}'. Choose from {list(solver_map.keys())}.")
        ...
        if inversion_type == 'map':
            ...
            x_posterior = solve_map_func(h_target_final, avg_noise_variance_final).reshape(1, -1)
            ...
            try:
                # Check if user already has a client running
                try:
                    client = get_client()
                ...
                delayed_x_maps = [map_solver_delayed(noisy_h_targets[k, :], avg_noise_variance_final)
                ...
                for k in range(n_posterior_samples):
                     ...
                     x_posterior_list.append(solve_map_func(noisy_h_targets[k, :], avg_noise_variance_final))
                x_posterior = np.array(x_posterior_list)
